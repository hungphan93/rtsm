...
public:
	...
	void on_cpu_changed(notify cb)
	{
		set_callback(cpu_, std::move(cb));
	}
	void on_memory_changed(notify cb)
	{
		set_callback(memory_, std::move(cb));
	}
	void on_gpu_changed(notify cb)
	{
		set_callback(gpu_, std::move(cb));
	}
	void on_disk_changed(notify cb)
	{
		set_callback(disk_, std::move(cb));
	}
	void on_net_changed(notify cb)
	{
		set_callback(net_, std::move(cb));
	}
...
private:
	...
	{
		net_view_model vm;
		vm.rx_speed = format_speed(
			v.rx_bytes); // Already calculated as Bytes/s in the Adapter
		vm.tx_speed = format_speed(
			v.tx_bytes); // Already calculated as Bytes/s in the Adapter
		update(net_, vm);
	}
